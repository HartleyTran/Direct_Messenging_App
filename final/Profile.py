...
class Post(dict):
    ...
    """
    def __init__(self, sender:str=None, entry:str = None, timestamp:float = 0):
        self.sender=sender
        ...
        dict.__init__(self, sender=self.sender, entry=self._entry, timestamp=self._timestamp)
...
class Profile:
        ...
        if os.path.exists(p) and p.suffix == '.dsu':
            try:
                ...
                for post_obj in obj['_posts']:
                    post = Post(post_obj['sender'], post_obj['entry'], post_obj['timestamp'])
                    self._posts.append(post)
                for dm_obj in obj['_messages']:
                    msg = DirectMessage(dm_obj['recipient'], dm_obj['entry'], dm_obj['timestamp'])
    ...
    def store_messages(self, messages:list[DirectMessage]) -> None:
        """
        Takes in list of directmessages and stores it in the profile
        """
